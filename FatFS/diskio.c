...
#define SPI_CLK         BIT2	// P2.2
#define SD_CS           BIT5	// P1.5
...
static void xmit_spi(BYTE dat){
	...
	UCB0TXBUF = dat;				// Write byte
	while(UCB0STATW & UCBUSY);

	UCB0RXBUF;					// Read to empty RX buffer, clear any ovverrun
...
static BYTE rcvr_spi (void){
	...
	UCB0IFG &= ~UCRXIFG;				// Ensure RXIFG clear
	while(!(UCB0IFG & UCTXIFG));			// Wait for TX ready
	UCB0TXBUF = 0xFF;				// Send dummy byte
	while(!(UCB0IFG & UCRXIFG));			// Wait for RX buffer

	ui8RcvDat = UCB0RXBUF;				// Read RX buffer
...
static void power_on (void){
	...
	*/
	...
	UCB0CTLW0 |= UCCKPL | UCMSB | UCMST | UCMODE_0 | UCSYNC;  //3-pin, 8-bit SPI master
	//Clock polarity select - The inactive state is high
	//MSB first
	UCB0CTLW0 |= UCSSEL_2;                          //Use SMCLK, keep RESET
	UCB0BR0 = 3;                                           //Initial SPI clock must be <400kHz
	UCB0BR1 = 0;                                            //f_UCxCLK = 1MHz/(3+1) = 250kHz
	UCB0CTLW0 &= ~UCSWRST;                                   //Release USCI state machine
	UCB0IFG &= ~UCRXIFG;
...
static void set_max_speed(void){
    UCB0CTLW0 |= UCSWRST;                                    //Put state machine in reset
    UCB0BR0 = 0;                                            //f_UCxCLK = 1MHz/(0+1) = 1MHz
    UCB0BR1 = 0;
    UCB0CTLW0 &= ~UCSWRST;                                   //Release USCI state machine
}// TODO: Check speeds
